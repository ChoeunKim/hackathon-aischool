...
async function recordUntilSilence({minMs=900, maxMs=6000, silenceDb=-52}={}){
  ...
  const tick = ()=>{
    ...
    if (db < silenceDb) { if (silentStart < 0) silentStart = now; }
    else { silentStart = -1; }
...
function mergeSlots(prev: Slots|null, next: Slots): Slots {
  const out: Slots = {
    ...
    veggies: Array.from(new Set([...(prev?.veggies||[]), ...(next.veggies||[])])),
    sauces:  Array.from(new Set([...(prev?.sauces||[]),  ...(next.sauces||[])])),
    exclude: Array.from(new Set([...(prev?.exclude||[]), ...(next.exclude||[])])),
  };
...
export default function Detail(){
  ...
  /** 무음감지 녹음 → /infer */
  const micOnce = async ()=>{
    const blob = await recordUntilSilence({minMs:900, maxMs:6000, silenceDb:-52});
